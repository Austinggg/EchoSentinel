...
@fact_check_api.route('/api/factcheck/claims', methods=['POST'])
...
@fact_check_api.route('/api/videos/<video_id>/factcheck/status', methods=['GET'])
def get_fact_check_status(video_id):
    """获取事实核查状态"""
    try:
        transcript = db.session.query(VideoTranscript).filter(VideoTranscript.video_id == video_id).first()
        if not transcript:
            return error_response(404, "未找到转录记录")
        
        status_data = {
            "video_id": video_id,
            "status": getattr(transcript, 'fact_check_status', 'pending'),
            "progress": 100 if getattr(transcript, 'fact_check_status', 'pending') == 'completed' else 
                      50 if getattr(transcript, 'fact_check_status', 'pending') == 'processing' else 0,
            "started_at": transcript.fact_check_timestamp.isoformat() if transcript.fact_check_timestamp else None,
            "error_message": getattr(transcript, 'fact_check_error', None)
        }
        
        return success_response(status_data)
        
    except Exception as e:
        logging.exception(f"获取事实核查状态失败: {str(e)}")

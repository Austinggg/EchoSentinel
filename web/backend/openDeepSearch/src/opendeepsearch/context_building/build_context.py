...
def build_context(
    ...
    try:
        ...
        if hasattr(sources_result, 'organic'):
            organic_results = extract_information(sources_result.organic)
        else:
            organic_results = extract_information([])
        if hasattr(sources_result, 'topStories'):
            top_stories = extract_top_stories(sources_result.topStories)
        else:
            top_stories = extract_top_stories(None)

        if hasattr(sources_result, 'answerBox'):
            answer_box = extract_answer_box(sources_result.answerBox)
        else:
            answer_box = extract_answer_box(None)

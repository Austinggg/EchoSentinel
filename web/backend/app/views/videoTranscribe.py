...
from app.views.videoUpload import get_video_file_path
from werkzeug.utils import secure_filename
from app.utils.database import db
from app.utils.HttpResponse import success_response, error_response
from app.services.fact_checking.FactCheckPipeline import get_fact_checker
import os
import tempfile
import logging
...
# 初始化转录器
from app.services.content_analysis.video_transcribe import VideoTranscriber  # 调整导入路径
...
def transcribe_video_by_id(file_id):
    try:
        from app.utils.database import VideoFile, VideoTranscript
...
def fact_check_video_transcript(file_id):
    """根据视频ID获取转录内容并进行事实核查"""
    try:
        from app.utils.database import VideoFile, VideoTranscript, FactCheckResult, db
        import datetime
        import logging
...
def save_transcript_to_db(video_id, result, video):
    """保存转录结果到数据库"""
    from app.utils.database import VideoTranscript, db

...
# ORM模型导入
from app.models.video import DouyinVideo
from app.models.task import UserAnalysisTask
from app.models.user import UserProfile
from app.utils.extensions import db
from app.views.workflow import WORKFLOW_TEMPLATES
...
def add_account():
    ...
    请求方式:
        POST
    请求数据 (JSON):
        - platform (str): 平台名称，必需字段。
        - platform_user_id (str): 平台用户ID，必需字段。
        - nickname (str): 用户昵称，必需字段。
        - signature (str, 可选): 用户签名。
        - gender (int, 可选): 性别。
        - city (str, 可选): 城市。
        - province (str, 可选): 省份。
        - country (str, 可选): 国家。
        - district (str, 可选): 区县。
        - aweme_count (int, 可选): 视频数。
        - follower_count (int, 可选): 粉丝数。
        - following_count (int, 可选): 关注数。
        - total_favorited (int, 可选): 获赞总数。
        - favoriting_count (int, 可选): 喜欢数。
        - user_age (int, 可选): 用户年龄。
        - ip_location (str, 可选): IP属地。
        - avatar_medium (str, 可选): 头像URL。
        - avatar (str, 可选): 头像URL（备用）。
        - show_favorite_list (bool, 可选): 是否展示喜欢列表。
        - is_gov_media_vip (bool, 可选): 是否为政务媒体。
        - is_mix_user (bool, 可选): 是否为混合用户。
        - is_star (bool, 可选): 是否为明星用户。
        - is_series_user (bool, 可选): 是否为系列用户。
        - custom_verify (str, 可选): 自定义认证信息。
        - enterprise_verify_reason (str, 可选): 企业认证原因。
        - verification_type (int, 可选): 认证类型。
        - sec_uid (str, 可选): 安全UID，若未提供则使用 platform_user_id。
    返回:
        - 成功:
            {
                "message": "账号添加成功" 或 "账号已存在，任务信息已更新",
                    "user_id": 用户ID,
                    "task_id": 任务ID,
                    "status": 任务状态 (仅账号已存在时返回),
                    "existing": 是否为已存在账号 (bool)
        - 失败:
            {
                "code": 400/500,
                "message": 错误信息
...
def get_video_processing_details(aweme_id):
    ...
    try:
        ...
        # 修改导入语句
        from app.models.task import ProcessingLog, VideoProcessingTask
...
def get_douyin_video_analysis_status(aweme_id):
    ...
    try:
        ...
        # 修改导入语句
        from app.models.analysis import ContentAnalysis
        from app.models.task import VideoProcessingTask

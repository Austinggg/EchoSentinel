...
from app.services.userAnalyse.AEModel import infer_8features, infer_loss
from app.services.userAnalyse.CoverFeature import ImageFeatureExtractor
from app.services.userAnalyse.data_processor import DataProcessor
from app.services.userAnalyse.OLSH import OLsh
...
def preprocess(df) -> pd.DataFrame:
    ...
    processor = DataProcessor.load("app/services/userAnalyse/data_processor.pkl")
...
def plot_data():
    csv_file = Path("app/services/userAnalyse/output8.csv")
    output_path = csv_file.parent / "tsne_with_labels.txt"
    index_file = Path("app/services/userAnalyse/olsh_index.joblib")
...
def get_feature_by_uid(sec_uid: str) -> np.ndarray:
    ...
    df = pd.read_csv("app/services/userAnalyse/output8.csv")

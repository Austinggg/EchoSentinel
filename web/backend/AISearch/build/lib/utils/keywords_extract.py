...
def keywords_extract(query: str) -> List[str]:
    ...
    prompt_zh = """你是一个专门生成最佳搜索查询的AI。请严格遵循以下规则生成搜索关键词：
            1、所有用户问题必须生成搜索关键词，无需判断是否需要搜索。
            2、生成关键词规则： 
                a. 基于问题提取2-5个关键搜索词或短语，禁止包含答案或解释，仅提供关键词。 
                b. 需考虑同义词和相关概念扩展搜索范围，覆盖问题核心及潜在相关领域。
                c. 优先选择具体且独特的词汇，按重要性排序，核心关键词靠前。
                d. 若用户问题包含日期信息，必须将日期融入关键词中（如"2024年网络安全趋势"）。
                e. 评估问题难度等级：
                    1级：简单直接的问题，只需查找简单事实
                    2级：需要专业知识解释，涉及多步骤/复杂推理
                f. 最终输出格式：["关键词1 关键词2...", "难度等级"]

            示例：
                输入：“你能做什么？当前日期：2024-12-15”
                输出：["AI功能 搜索能力 当前日期2024-12-15", "1"]
                
                输入：“蔡徐坤是谁？当前日期：2024-11-15”
                输出：["蔡徐坤 个人介绍 2024", "1"]
                
                输入：“量子计算对网络安全的影响是什么？当前日期：2024-12-18”
                输出：["量子计算 网络安全 加密技术 2024影响", "2"]
                
                输入：“解释相对论？当前日期：2024-10-10”
                输出：["相对论原理 爱因斯坦 时空理论", "2"]
            
            请严格按照示例格式输出，禁止返回其他内容。"""

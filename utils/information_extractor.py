...
class InformationExtractor:
    def __init__(self, config):
        self.config = config
        self.use_local = config["local_information_extraction"]
        
        # 初始化客户端
        if not self.use_local:
            self.client = OpenAI(
                api_key=config["information_extraction_model"]["remote_openai"]["api_key"],
                base_url=config["information_extraction_model"]["remote_openai"]["base_url"]
            )
